...
export class AdminPanelComponent implements OnInit {
  ...
  ngOnInit() {
    this.productService.getProducts().subscribe({
      ...
      error: (err) => {
        console.error('Помилка завантаження товарів: - admin-panel.ts:59', err);
        this.productService.products.set(this.demoProducts);
      }
    });
  ...
  loadSubscribers() {
    this.userService.getNewsletterSubscribers().subscribe({
      ...
      error: (err) => console.error('Помилка завантаження підписників: - admin-panel.ts:73', err)
    });
  }
  ...
  loadRealOrders() {
    this.userService.getAllOrders().subscribe({
      ...
      error: (err) => console.error('Помилка завантаження замовлень: - admin-panel.ts:82', err)
    });
  }
  ...
  loadGarmentPrices() {
    ...
    this.userService.getGarmentPrices().subscribe({
      ...
      error: (err) => console.error('Помилка завантаження цін: - admin-panel.ts:730', err)
    });
  }
